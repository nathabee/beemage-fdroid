...
export type PipelineVm = {
  ...
  /**
   * NEW: full input artifact (so view can show imageList count etc.)
   */
  inputArtifact?: Artifact;

  /**
   * Legacy input preview slot (image only), still used by current view layout.
   */
  ...
  /**
   * NEW: pipeline outputs that are not canvas-friendly
   */
...
export type PipelineModel = {
  ...
  // Back-compat: existing callers can keep using this.
...
function isImageList(a: Artifact): a is Extract<Artifact, { type: "imageList" }> {
  return a.type === "imageList";
}
function isPdf(a: Artifact): a is Extract<Artifact, { type: "pdf" }> {
  return a.type === "pdf";
}
...
export function createPipelineModel(deps: PipelineModelDeps): PipelineModel {
  ...
  async function reloadCatalogue(): Promise<void> {
    ...
    // invalidate step plan (spec refs can go stale)
    plan = [];
    nextIndex = 0;
  }
  ...
  function makeRecipesForPipeline(pipeline: PipelineDef): RecipeDef[] {
    ...
    return defs.slice(0, 1).concat(defs.slice(1).sort((a, b) => String(a.title).localeCompare(String(b.title))));
  }
  ...
  function buildPlanFromPipeline(p: PipelineDef): PlannedStep[] {
    ...
    for (let i = 0; i < enabled.length; i++) {
      ...
      if (!spec) continue;
      out.push({ index0: i, instanceId: inst.instanceId, inst, opSpec: spec });
    }
    ...
    return [
      {
        ...
        output: stageOut?.type ?? result.input.type,
        ops,
        state: stageState,
        error: result.error,
  ...
  function applyOutputsToVm(vm: PipelineVm, out: Artifact | null): void {
    // clear all outputs each render (so UI doesn't show stale type if a different pipeline ran)
    vm.outputImage = undefined;
    vm.outputMask = undefined;
    vm.outputSvg = undefined;
    vm.outputImageList = undefined;
    vm.outputPdf = undefined;
    ...
    if (isImage(out)) {
      ...
      vm.outputPdf = { bytes: out.bytes, filenameHint: out.filenameHint };
    }
  }
  ...
  async function runNext(): Promise<void> {
    ...
    if (!currentArtifact) currentArtifact = input;
    ...
    try {
      ...
      if (spec.kind === "dispatch") {
        ...
        const override = step.inst.override;
        ...
        if (currentArtifact.type === "image") {
          const raw = await runOp(
            ...
            override ? { enginePolicy: override.enginePolicy, params: { ...params, ...(override.params ?? {}) } } : { params },
          );
        ...
        // JS op: we merge tuning params + instance override.params (if any)
        const mergedParams = {
          ...params,
          ...(step.inst.override?.params ?? {}),
        };
        out = await spec.run({ input: currentArtifact, params: mergedParams });
      }
  ...
  function getVm(): PipelineVm {
    ...
    const vm: PipelineVm = {
      ...
      inputArtifact: input ?? undefined,

      // legacy input preview (image only)
      input: input && input.type === "image" ? { width: input.width, height: input.height, data: input.image } : undefined,
