...
export function createPipelinePlayground(args: {
    ...
    const startType: ArtifactType = "image";

    let noticeKind: "info" | "error";
    ...
    function canDropAt(index: number, opId: string): { ok: boolean; reason?: string } {
        ...
        const beforeType = typeAtIndex({ specs, startType, index });
    ...
    function saveNow(): void {
        ...
        // Minimal safety: verify adjacency typing.
        let cur: ArtifactType = startType;
    ...
    function render(): void {
        ...
        // Typing summary
        const specs = resolveDraftSpecs(draft, opById);
        const endType = computeEndType(specs, startType);

        const summary = el(
            "div",
            { class: "muted", style: "font-size:12px; margin-top:10px;" },
            `Start: ${startType}  →  Current output: ${endType}`,
        );
        ...
        function makeDropSlot(index: number): HTMLDivElement {
            ...
            slot.addEventListener("dragover", (ev) => {
                ...
                // IMPORTANT: always allow drop during dragover.
                // Chrome may not expose getData() here reliably.
                if (ev.dataTransfer) ev.dataTransfer.dropEffect = "copy";

                // Optional: if we *can* read opId, show red/green hint, but never block the drop here.
